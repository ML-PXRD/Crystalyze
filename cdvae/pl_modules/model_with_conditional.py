...
#make an empty dictionary called lookup_dict
lookup_dict = {}
...
class CDVAE(BaseModule):
    ...
    def lookup_diffraction(data_in_list_version):
        
        def tensor_to_list(tensor_data):
            # If data is tensor, move to CPU and convert to list
            if isinstance(tensor_data, torch.Tensor):
                return tensor_data.cpu().tolist()
            return tensor_data

        # Atomic species
        data = data_in_list_version

        if isinstance(data.atom_types[0], torch.Tensor):
            atomic_species = [Element.from_Z(atom_type.item()).symbol for atom_type in data.atom_types.cpu()]
        else:
            atomic_species = tensor_to_list(data.atom_types)

        frac_coordinates = tensor_to_list(data.frac_coords)
        angles = tensor_to_list(data.angles)
        lengths = tensor_to_list(data.lengths)

        # print("The atomic species is: ", atomic_species)
        # print("The frac coordinates are: ", frac_coordinates)

        alpha, beta, gamma = angles[0]
        # print("The data angle object is ", angles)
        # print("The first element of the data angle object is ", angles[0])
        # print("The angles are: ", alpha, beta, gamma)

        a, b, c = lengths[0]
        # print("The data length object is ", lengths)
        # print("The first element of the data length object is ", lengths[0])
        # print("The lengths are: ", a, b, c)

        #make an ID with all the information just concatenated together
        ID = str(atomic_species) + str(frac_coordinates) + str(angles) + str(lengths)

        # look through the global dictionary to see if the ID is already there
        if ID in lookup_dict:
            #return the value of the ID
            return lookup_dict[ID]
        #if the ID is not there, calculate the diffraction pattern and add it to the dictionary
        else:
            ...
            # print("The xrd_calculator is: ", xrd_calculator)

            pattern = xrd_calculator.get_pattern(structure)
            # print("The pattern is: ", pattern)

            #adjust the pattern to have 256 values with 0 padding
            if len(pattern.x) < 256:
                adjusted_vector = np.zeros(256)

                #minimum value between 256 and the length of the pattern
                minimum = min(256, len(pattern.x))
                
                adjusted_vector[:minimum] = pattern.x[:minimum]

                # print("The adjusted vector is: ", adjusted_vector)

            #add the atom type information to end of the adjusted vector. overwrite any values that are already there
            atomic_numbers = [Element(symbol).Z for symbol in atomic_species]
            for i in range(len(atomic_numbers)):
                #add the atomic number to the back of the vector
                adjusted_vector[-i] = atomic_numbers[i]

            #add the adjusted vector to the dictionary
            lookup_dict[ID] = adjusted_vector
    ...
    def encode(self, batch):
        ...
        for data in list_verion:
            ...
            peak_locations.append(self.lookup_diffraction(data))

            ### OLD CODE W/O DICTIONARY LOOKUP ###
            ...
            # #add the adjusted vector to the list of peak locations as a list of lists, not concatenated
            # peak_locations.append(adjusted_vector)
            # # print("The peak locations are: ", peak_locations)

        #convert the list of lists to a tensor
        peak_locations_tensor = torch.tensor(peak_locations)

        # print("The peak locations tensor is: ", peak_locations_tensor)

        # Transfer tensor to the GPU
        peak_locations_tensor = peak_locations_tensor.to('cuda:0')

        # print("The peak locations tensor after transferring to the GPU is: ", peak_locations_tensor)
        
        peak_locations_tensor_float32 = peak_locations_tensor.to(torch.float32)
        z = peak_locations_tensor_float32

        hidden = self.encoder(batch)

        #add z to the hidden layer via element by element addition
        hidden = torch.add(hidden, z)

        # print("The hidden is: ", hidden)
    ...
    def get_prior_dist(self, pdf_cond):
        ...
        prior_layers = self.conditioning_nw(model_arch['PDF_len'], model_arch['posterior'], model_arch['mlps']['m0']) 

        for idx, layer in enumerate(prior_layers):
            if idx == len(prior_layers) - 1:
                cond_prior = layer(cond_prior)
            else:
                cond_prior = nn.LeakyReLU((layer(cond_prior)))

        cond_prior = cond_prior.squeeze(-1)
        #squeezing the cond_prior to get rid of the last dimension
    ...
    def forward(self, batch, teacher_forcing, training):
        ...
        #added by Tsach: calculate the KL divergence
        sigma = torch.exp(log_var / 2) * self.sigma_scale

        posterior_distribution = Independent(Normal(loc=mu, scale=sigma), 2)
