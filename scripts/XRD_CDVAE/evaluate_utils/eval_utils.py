...
def all_results_retreival(recon_file_path, num_batches = 1, label = ""):
   all_results = []
   all_gt = []
...
def evaluation(all_results, all_gt, set_size = 256, num_batches = 1, stol=0.5, angle_tol=10, ltol=0.3):
   total_rmsd = []
   total_rmsd_just_sites = []

   for batch_idx in tqdm(range(num_batches)):
        all_results_crystals = [Crystal(x) for x in all_results[batch_idx]]
        all_gt_crystals = [Crystal(x) for x in all_gt[batch_idx]]
        ...
        try:
            ...
            print('error is ', e)
            recon_metrics = {'rmsd_values': len(all_gt[batch_idx]) * [None]}
...
def symmetry_performance(pred_crystal_list, gt_crystal_list, tolmax = 2):
    pred_spacegroups = []
    pred_tolerance = []
    gt_spacegroups = []

    for i in tqdm(range(len(pred_crystal_list))): 
        pred_found = False
        ...
        gt_spacegroups.append(gt_sg_num)
    ...
    gt_spacegroups = np.array(gt_spacegroups)

    #print the shapes of the arrays 
    print(pred_spacegroups.shape)
    print(pred_tolerance.shape)
    print(gt_spacegroups.shape)

    return pred_spacegroups, pred_tolerance, gt_spacegroups

def tolerance_analysis(prediction, ground_truth, ltol_values, angle_tol_values, set_size, num_batches):
    ...
    for ltol in tqdm(ltol_values):
        evals_per_ltol.append(evaluation(prediction, ground_truth, set_size, num_batches = num_batches, ltol=ltol)[0])

    evals_per_angle_tol = []
    for angle_tol in tqdm(angle_tol_values):
        evals_per_angle_tol.append(evaluation(prediction, ground_truth, set_size, num_batches = num_batches, angle_tol=angle_tol)[0])
...
def load_and_merge(data_dir, label, total_data = None, num_workers = 100): 
    for worker in range(num_workers): 
        ...
        if worker == 0: 
            ...
            if len(total_data_chunk.shape) == 3:
